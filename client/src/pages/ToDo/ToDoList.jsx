...
function ToDoList() {
  ...
  return (
    <>
    <Navbar active={"myTask"}></Navbar>
    <section className={styles.toDoWrapper}>
      <div className={styles.toDoHeader}>
          <h2>Your Task</h2>
            <Input style={{width:'50%'}} onChange={handleSearch} placeholder='Search Your ZueTask Here...'/>
            <div>
            ...
            <div key={item?._id} className={styles.toDoCard}>
              ...
              <div className={styles.toDoCardFooter}>
                ...
                <div className={styles.toDoCardFooterAction}>
                  ...
                  <Tooltip title="Delete Task!?"><DeleteOutlined onClick={()=>handleDelete(item)}
                    style={{color:'red'}} className={styles.actionIcon}/></Tooltip>
                    {item?.isCompleted ? <Tooltip title="Mark as Incomplete">
                      <CheckCircleFilled onClick={()=>handleUpdateStatus(item._id,false)} style={{color:'green'}}
                        className={styles.actionIcon}/> </Tooltip> :<Tooltip title="Mark as Completed">
              ...
              <div className={styles.toDoCardFooter}>
                 ...
                 <div className={styles.toDoCardFooterAction}>
                  ...
                  {item?.isCompleted ? <Tooltip title="Mark as Incomplete">
                    ...
                    </Tooltip> : <Tooltip title="Mark as Completed">
                      <CheckCircleOutlined onClick={()=>handleUpdateStatus(item._id,true)} className={styles.actionIcon}/> </Tooltip>}
                 </div>
              </div>
      ...
      <Modal confirmLoading={loading} title="Add New ToDo Task" open={isAdding} onOk={handleSubmitTask} onCancel={()=>setIsAdding(false)}>
        <Input style={{marginBottom:'1rem'}} placeholder='Title' value={title} onChange={(e)=>setTitle(e.target.value)}></Input>
      ...
      <Modal confirmLoading={loading} title={`Update ${currentEditItem.title}`} open={isEditing} onOk={handleUpdateTask} onCancel={()=>setIsAdding(false)}>
      <Input style={{marginBottom:'1rem'}} placeholder='Updated Title' value={updatedTitle} onChange={(e)=>setUpdatedTitle(e.target.value)}></Input>
